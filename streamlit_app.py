...
<style>
    ...
    * { font-family: 'Poppins', sans-serif; }
    ...
    .main .block-container {
        ...
        border-radius: 16px; padding: 25px; border: 1.5px solid rgba(255, 255, 255, 0.18);
        box-shadow: 0 8px 32px rgba(0, 0, 0, 0.1);
    }
    .main-header {
        background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(12px);
        padding: 1.5rem; border-radius: 16px; text-align: center; margin-bottom: 1.5rem;
    ...
    .stButton>button {
        background: linear-gradient(45deg, #ff6b6b, #4ecdc4);
        color: white; border: none; border-radius: 12px; padding: 0.7rem 1.8rem;
    ...
    .stTextInput>div>div>input, .stTextArea>div>div>textarea, .stNumberInput>div>div>input {
        ...
        border-radius: 10px; color: white; padding: 0.8rem; transition: all 0.3s ease;
    }
    .stTextInput>div>div>input:focus, .stTextArea>div>div>textarea:focus, .stNumberInput>div>div>input:focus {
        background: rgba(255, 255, 255, 0.15); border-color: rgba(78, 205, 196, 0.6);
        box-shadow: 0 0 12px rgba(78, 205, 196, 0.2);
    }
    label { color: #ffffff !important; font-weight: 500 !important; font-size: 14px !important; }
    .console-box {
...
def run_session(session, post_id, cookies_list, comments_data, prefix, delay):
    retries = 0
    driver = None
    comments_list = comments_data.get('comments', ['Nice post!']) if isinstance(comments_data, dict) else []
    ...
    while session.running and retries < 5:
        try:
            ...
            # Add cookies from first account
            if cookies_list:
                session.log(f'Using cookies ({len(cookies_list)} account(s))...')
                for c in cookies_list[0].split(';'):
                    c = c.strip()
                    if c and '=' in c:
                        i = c.find('=')
                        try:
            ...
            while session.running:
                try:
                    ...
                    # Extract comment text and image if exists
                    comment_text = comment_to_send
                    has_image = 'images' in comments_data and comments_data['images']
                    
                    session.log(f'Typing: {comment_text[:25]}...' + (' (with image)' if has_image else ''))
...
def start_session(session, post_id, cookies_list, comments_data, prefix, delay):
    ...
    if isinstance(comments_data, str):
        comments_data = {'comments': [c.strip() for c in comments_data.split('\n') if c.strip()] or ['Nice post!']}
    
    comments_list = comments_data.get('comments', ['Nice post!'])
    threading.Thread(target=run_session, args=(session, post_id, cookies_list, comments_data, prefix, delay), daemon=True).start()
...
if st.session_state.view_session:
    ...
    # Multi-cookie input section
    st.markdown("#### Cookies (Multi-Account)")
    cookie_input_method = st.radio("Choose cookie input method:", ["Direct Input", "Upload Cookie File"], horizontal=True)
    
    cookies_list = []
    if cookie_input_method == "Direct Input":
        num_accounts = st.number_input("Number of accounts:", 1, 10, 1)
        for i in range(num_accounts):
            cookie = st.text_area(f"Account {i+1} Cookies", height=80, placeholder="Paste cookies here... (semicolon separated)", key=f"cookies_{i}")
            if cookie.strip():
                cookies_list.append(cookie.strip())
    else:
        cookie_file = st.file_uploader("Upload Cookies File (txt)", type=['txt'], key="cookie_file")
        if cookie_file:
            file_content = cookie_file.read().decode('utf-8')
            for line in file_content.split('\n'):
                if line.strip() and not line.startswith('#'):
                    cookies_list.append(line.strip())
    
    # Comments section with optional image
    st.markdown("#### Comments")
    uploaded_comments = st.file_uploader("Upload Comments TXT", type=['txt'], key="comments_file")
    ...
    # Optional image upload for comments
    st.markdown("#### Optional - Images with Comments")
    image_option = st.checkbox("Add image to comments?")
    uploaded_image = None
    if image_option:
        uploaded_image = st.file_uploader("Upload Image", type=['jpg', 'jpeg', 'png', 'gif'], key="comment_image")
        if uploaded_image:
            st.image(uploaded_image, width=150)
    
    # Prepare comments data with optional image
    comments_data = {'comments': [c.strip() for c in comments.split('\n') if c.strip()] or ['Nice post!']}
    if uploaded_image:
        comments_data['images'] = [uploaded_image.read()]
    ...
    if st.button("START NEW SESSION", use_container_width=True):
        if not cookies_list:
            ...
            start_session(new_session, post_id, cookies_list, comments_data, prefix, delay)
